...
class Environment():
    def __init__(self, Design):
        ...
        #params
        self.patient_max_num = 10 #4000#5 #10  #maximum number of patients who can come for therapy
        ...
        self.time_budget_for_Arrival =100  #total time for patient arrival in a year's time
        self.Simulation_time_budget = 500     #total time for patient arrival in a year's time
        self.Female_ratio = 0.5
        #self.bad_pat_rate = 2
        #self.avg_pat_rate = 5
        #self.good_pat_rate = 3
        
        #params of design: [System mix, Patient mix, QM Policy, Hrv Operator, Hrv Machines, Mfg Operators, Mfg Machines]
        #Design to params
        if Design[0] == 0:
            #stressed system
            self.mfg_dura_perc = [0.5, 0.25, 0.25]
            self.mfg_dura = [7, 20, 35]
            self.curve_sap = [10, 30, 55]
        elif Design[0] == 1:
            #relaxed system
            self.mfg_dura_perc = [0.25, 0.5, 0.25]
            self.mfg_dura = [20, 35, 60]
            self.curve_sap = [25, 50, 70]
        #Paitient mix
        if Design[1] == 0:
            self.bad_pat_rate = 2
            self.avg_pat_rate = 5
    ...
    def Simulate(self):
        ...
        """
        output_file = 'Event_information.csv'
        header = ['Clock', 'Event name', 'Event type', 'Event happen time', 'Event place', 'Event machine', 'Event operator', 'Event job', 
                  'Job yield', 'hrv_operator_state_list', 'mfg_operator_state_list', 'hrv_machine_state_list', 'mfg_machine_state_list', 
                  'q1_state_list', 'q2_state_list', 'q3_state_list', 'q4_state_list', 'q5_state_list', 'job_state_list','event_list','total job count']
        ...
        self.df_this_design = pd.DataFrame(columns=['Clock', 'Event name', 'Event type', 'Event happen time', 'Event place', 'Event machine', 'Event operator', 'Event job', 
                                                    'Job yield', 'hrv_operator_state_list', 'mfg_operator_state_list', 
                                                    'hrv_machine_state_list', 'mfg_machine_state_list', 'q1_state_list', 'q2_state_list', 'q3_state_list', 
                                                    'q4_state_list', 'q5_state_list', 'job_state_list','event_list','total job count'])
        ...
        while self.clock <= self.Simulation_time_budget:
            ...
            if self.event_list == []:
                ...
                to_append = [self.clock]+ this_event.get_event_info() + self.get_current_state() + [self.job_gen_count]
                Put_something_into_csv(to_append, output_file)

            #self.df_this_design = self.df_this_design.append(pd.Series(to_append, index = self.df_this_design.columns), ignore_index=True)  #df csv
    ...
    def Processing_yield_curve(self, dura, job):
        ...
        t_1 = self.curve_sap[0]
        t_2 = self.curve_sap[1]
        t_3 = self.curve_sap[2]
        y_bar = job.patients_target_bc*(1 + np.random.normal())
        k_1 = y_bar/t_1
        k_2 = - y_bar/(t_3-t_2)
        if dura <= t_1:
            p_yield = k_1*dura
        elif dura > t_1 and dura <= t_2:
            p_yield = y_bar
        elif dura > t_2 and dura <= t_3:
            p_yield = y_bar + k_2*dura
        elif dura > t_3:
            p_yield = 0
        #if job.pt_type == 'bad':
        #    pp_yield = np.random.triangular(0.5*p_yield, 0.675*p_yield, 0.85*p_yield)
        #elif job.pt_type == 'avg':
        #    
    ...
    def Process_event(self, this_event):
        ...
        if this_event.e_type == 'Arrival':
            ...
            #schedule next
            next_event = toolbox.Event('Job {} end processing at {}'.format(this_job.name, this_event.machine.name), 'End_Processing', self.clock+processing_duration, this_event.place, this_event.machine, None, this_job)
            self.add_event(next_event)
            ...
            if this_job.processing_yield <= 0:
                ...
                final_yield = self.Processing_yield_curve(duration, this_job)
